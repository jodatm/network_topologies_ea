...
while(condition and number_of_colors > 0):
	...
	while average_calculation != 0:
		...
		G = nx.generators.random_graphs.watts_strogatz_graph(population_size,3,rewiring)		
		...
		while(best_fitness != 0 and gen != 10000): 
			...
			selection_pool_size = len(population)
			G = nx.generators.random_graphs.watts_strogatz_graph(selection_pool_size,6,rewiring)
			for i in range(population_size): 
				individual_i = population[i]
				best_neighbor = 0
				best_neighbor_fitness = 100000000
				for k in G[i]:
					neighbor_fitness = fitness(graph=graph, individual=population[k], graph_size=n)
					if(neighbor_fitness<best_neighbor_fitness):
						best_neighbor_fitness = neighbor_fitness
					best_neighbor = k
				if(best_neighbor_fitness<100000000):
					mate = population[best_neighbor]					
					child1, child2 = crossover(parent1=individual_i, parent2=mate, graph_size=n)
